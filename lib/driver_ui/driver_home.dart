  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar:    AppBar(
                ...
                elevation: 0,
                actions: [  
                  rideStatus==0||rideStatus==1||rideStatus==2||rideStatus==5?
                  Container(
         margin: EdgeInsets.all(10),
          child:  FlutterSwitch(
            ...
            inactiveIcon: Icon(Icons.remove_circle_outline_outlined,color:Colors.redAccent),
            value: onlineStatus,
            borderRadius: 30.0,
            padding: 3.0,
            ...
            onToggle: (val) {
              setState(() {
                ...
                leading: GestureDetector(onTap: (){_scaffoldKey.currentState.openDrawer();}, child: Icon(Icons.menu, color: DarkGray,),),
               
      ),
      body:isLoading==true?Center(child: CircularProgressIndicator(),): DoubleBackToCloseApp(
          child:  Stack(
        children: [
          mapWidget(),
          _controllLayer(),
  ...
  Widget _controllLayer(){
    return Container(
      ...
      child: Column(
        ...
        children: [
       //   statusChangeBar(),
           rideStatus==4||rideStatus==2?Container(
             margin: EdgeInsets.only(left: 10, right: 10),
            padding: EdgeInsets.all(10),
            color: Colors.black,
            child: Row(
              children: [
                Icon(Icons.assistant_direction,size : 30,color: Colors.white,),
                SizedBox(width: 10,),
                Flexible(
                  child: Text(
                    rideStatus==4
   ...
   Widget requestLocation(){
    return GestureDetector(
      child: Container(
        ...
        child: Text('Decline', style: TextStyle(color: whtColor,fontSize: 14),),
      ),
    );
  }
...
Widget statusButton(){
          ...
          AnimatedPositioned(
            ...
            child: GestureDetector(
              ...
              child: AnimatedSwitcher(
                ...
                child: onlineStatus? Icon(Icons.check_circle, color: green,size: 25,key: UniqueKey(),):Icon(Icons.remove_circle_outline, color: redColor,size: 25,key: UniqueKey(),),
              ),
            ),
          )
        ],
  ...
  Widget bottomWidgetChanger(){
    return onlineStatus? rideStatus==0?_ifOnline():rideStatus==1?driverOnTheWay(): rideStatus==2?driverOnTheWay() :driverOnTheWay() :_ifOffline();
  }
  ...
  Widget rideStatusRequest(){
    return Container(
      ...
      child: Column(
        children: [
          Text('Estimated Fair  QAR 15', style: TextStyle(fontSize: 20, backgroundColor: whtColor, fontWeight: FontWeight.bold),),
          PrimaryButton(FillColor: green,Heading: 'Accept',onTap: acceptPressed,)
        ],
      ),
    );
  }
  ...
  errorWidget: (context, url, error) => Icon(Icons.error),
                    ...
                    Expanded(
                      ...
                      style: TextStyle(
                        ...
                        Text(tripList[0].rider_name==null?'Customer Name':tripList[0].rider_name,
                      overflow: TextOverflow.visible, 
                      style: TextStyle(
                        color: DarkGray,
                        fontSize: 16,
                ...
                tripDirectionDetails==null?
                  Container(
                      ...
                      color: rideStatus==5?Colors.red:Colors.black,
                      child: Text(rideStatus==5?'20min\n7km':rideStatus==3?'1:50\nWaiting':'10min\n2km',textAlign: TextAlign.center, style: TextStyle(color: whtColor, fontSize: 16, fontWeight: FontWeight.bold)),
                    )
                  :Container(
                      margin: EdgeInsets.only(left: 10,),
                      padding: EdgeInsets.only(left:20,right: 20,top:5,bottom:5),
                      color: rideStatus==5?Colors.red:Colors.black,
                      child: Text(tripDirectionDetails.durationText+'\n'+tripDirectionDetails.distanceText,textAlign: TextAlign.center, style: TextStyle(color: whtColor, fontSize: 16, fontWeight: FontWeight.bold)),
                    )
                  :SizedBox()  
                ...
                Row(
                 crossAxisAlignment: CrossAxisAlignment.start,
                  ...
                  Image.asset('assets/desticon.png',height: 16,width: 16,),
                    ...
                    SizedBox(width:10),
                     ...
                     child:  Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                         children: [
                           SizedBox(height: 12,),
                          Expanded(child: Text(tripList[0].pickup_address==null? 'Barkat wala gala, Bismillah Colony, Gujranwala':tripList[0].pickup_address,overflow: TextOverflow.clip,maxLines: 2, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold),),),
                           SizedBox(height: 12,),
                         Expanded(child: Text(tripList[0].destination_address==null?'Appt. no 32, Phase IV, Street 59, Ring Road Lahore':tripList[0].destination_address,overflow: TextOverflow.clip,maxLines: 2, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold),),),
                   ...
                   rideStatus>1&&rideStatus<4?BrandDivider():SizedBox(),
                SizedBox(height: 10,),
                 rideStatus>1&&rideStatus<6?_callMessageButton():SizedBox(),
          ...
          rideStatus==1?PrimaryButton(FillColor: green,Heading: 'Accept',onTap: acceptPressed,):
          rideStatus==2?PrimaryButton(FillColor: green,Heading: 'Arrived',onTap: arrivedPress,):
          rideStatus==3?PrimaryButton(FillColor: green,Heading: 'Start Trip',onTap: startRidePress,):
          rideStatus==4?PrimaryButton(FillColor: Colors.red,Heading: 'Complete Trip',onTap: rideCompletPress,):
          rideStatus==5?PrimaryButton(FillColor: green,Heading: 'Calculate Fare',onTap: collectCash):SizedBox(),
          rideStatus==1||rideStatus==2?FlatButton(child: Text('Decline',style:TextStyle(color: Colors.red)),onPressed: declinePressed,):SizedBox(),
...
getUserData(){
  ...
  Geofire.setLocation(user.uid, currentLocation["latitude"], currentLocation["longitude"]);
               }
     });
        rideReference = FirebaseDatabase.instance.reference().child('users/${user.uid}/newReq');
  await  checkRequest();
    } on Exception {
...
Future<void> geoOnline(status) async {
  ...
  tripRequestRef = FirebaseDatabase.instance.reference().child('users/${user.uid}/newtrip');
...
void getCurrentDriverInfo() async{
  ...
  //   p.getToken();
 User user = FirebaseAuth.instance.currentUser;
  DatabaseReference driverRef = FirebaseDatabase.instance.reference().child("users/${user.uid}");
...
//5 = calculate fare
  ...
  // add driver details
 DatabaseReference rideRef = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}');
    ...
    rideRef.child('status').set(1);
             DatabaseReference   tripRef = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}');
               ...
               var trip = Trip(
                     ...
                     tripList.add(trip);
                          print(tripList.length);
                            DatabaseReference updat = FirebaseDatabase.instance.reference().child('users/${custom_rider_id}');
   ...
   getLocationUpdates();
      ...
      DatabaseReference checkDriverLocation = FirebaseDatabase.instance.reference().child('users/${user.uid}/position');
...
DatabaseReference rideRef = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}');
  ...
  void endTrip()async{
   ...
   var destinationLntLng = LatLng(pickup.longitude,pickup.latitude);
    ...
    var directionDetails = await HelperMethods.getDirectionDetails(destinationLntLng, currentLatLng);
  ...
  void getLocationUpdates(){
        ridePositionStream = Geolocator.getPositionStream(desiredAccuracy: LocationAccuracy.bestForNavigation,distanceFilter: 4,forceAndroidLocationManager: true).listen((event) {
        myPosition = position;
...
DatabaseReference rideRef = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}');
  ...
  Future<void> updateTripDetails() async {
    ...
    destinationLntLng = LatLng(pickup.longitude,pickup.latitude);
      ...
      destinationLntLng = LatLng(tripList[0].destination_latitude,tripList[0].destination_longitude);

    
    }

    var directionDetails = await HelperMethods.getDirectionDetails(positionLatLng, destinationLntLng);
 ...
 User user = FirebaseAuth.instance.currentUser;
    ...
    rideReference.remove();
        DatabaseReference teRefs = FirebaseDatabase.instance.reference().child('users/${user.uid}');
...
DatabaseReference tripHistory = FirebaseDatabase.instance.reference().child('tripHistory/${trip_id}/status');
      tripHistory.set(9);
     DatabaseReference teRef = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}/status');
              teRef.set(9);  
 //  HelperMethods.disableHomeTabLocationUpdates();
  ...
  //  Ringtone.play();
     DatabaseReference teRef = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}/status');
              teRef.set(4);  
   DatabaseReference complaint = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}');
...
void checkResume(){
    ...
    try{
     ...
     rideReference.onValue.listen((event) {
        ...
        if(event.snapshot!=null){
          ...
          tripReference = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}');
           tripReference.once().then((DataSnapshot dataSnapshot) async {
              if(dataSnapshot.value!=null){
                    print(dataSnapshot.value.toString());
                
              print('${dataSnapshot.value["destination"]["longitude"]}, ${dataSnapshot.value["destination"]["latitude"]}'); 
         DatabaseReference   tripRef = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}/request_to_status');
           ...
           setState(() {
               ...
               driver_feed = event.value["feedback"];
                ...
                // if //status
                    ...
                    if(b.value==1){
                       DatabaseReference teRefer = FirebaseDatabase.instance.reference().child('rideRequest/${trip_id}/request_to_status');
              teRefer.set(1); 
                      acceptPressed();
                    }else if(b.value==2){
                      declinePressed();
 ...
 Future<void> getDirectiontoCustomer() async {
  ...
  Marker pickupMarker = Marker(
    ...
    icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueAzure),
    infoWindow: InfoWindow(title: pickup.placeName,snippet: 'Driver location'),
  );
 ...
 setState(() {
  ...
  Marker destinationMarker = Marker(
    ...
    position: destinationLatLng,
    icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueYellow),
    infoWindow: InfoWindow(title: tripList[0].pickup_address,snippet: 'Rider Destination'),
  );
...
setState(() {
  ...
  Marker destinationMarker = Marker(
    ...
    position: destinationLatLng,
    icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueYellow),
    infoWindow: InfoWindow(title: tripList[0].destination_address,snippet: 'Rider Destination'),
  );
