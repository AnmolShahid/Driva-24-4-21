...
class CustomerTripHistoryClass extends StatefulWidget {
  @override
  _CustomerTripHistoryClassState createState() => _CustomerTripHistoryClassState();
}
 ...
 StreamBuilder(
    ...
    if(snap.data.snapshot.value!=null){
      ...
      onTap: ()async{
        tripHistory.clear();
        if(constant_role==0){
          DatabaseReference updateRef = FirebaseDatabase.instance.reference().child('users/${list[index]["driver_id"]}');
        ...
        DatabaseReference updateRef = FirebaseDatabase.instance.reference().child('users/${list[index]["rider_id"]}');
           updateRef.once().then(( DataSnapshot event) async {
           setState(() {
               driver_img = event.value["displayImage"];
        ...
        var  _tripHistory = Trip(
                        ...
                        destination_address: list[index]["destination_address"],
                        destination_latitude: list[index]["destination"]["latitude"],
                        destination_longitude: list[index]["destination"]["longitude"],
                        driver_id: list[index]["driver_id"],
                        pickup_address: list[index]["pickup_address"],
                        pickup_latitude: list[index]["location"]["latitude"],
                        pickup_longitude: list[index]["location"]["longitude"],
                        ...
                        driver_latitude: list[index]["driver_location"]["latitude"],
                        driver_longitude: list[index]["driver_location"]["longitude"],
                        total_EstimatedFare: list[index]["total_EstimatedFare"].toString(),
                        base_EstimatedFare: list[index]["base_EstimatedFare"].toString(),
                        distance_EstimatedFare: list[index]["distance_EstimatedFare"].toString(),
                        time_EstimatedFare: list[index]["time_EstimatedFare"].toString(),
                        distance: list[index]["distance"].toString(),
                        ride_start: list[index]["ride_start"],
                        ride_end: list[index]["ride_end"],
                        complaintByCustomer:list[index]["complaintByCustomer"],
                        complaintByDriver:list[index]["complaintByDriver"],
                        feedback_to_driver: list[index]["feedback_to_driver"],
                        feedback_to_customer: list[index]["feedback_to_customer"],
                    );
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Column(
                  ...
                  children: [
                   Text(
                     constant_role==0?
                     list[index]["driver_name"]??'Driver'
                     :
                     list[index]["rider_name"]??'Customer'
                     , style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800),),

                   Container(
                    child: Text(list[index]["date"]??'', style: TextStyle(fontSize: 12, ),),
                ),
                Container(
             ...
             //    Flexible(child:Text('Trip ID: ${list[index]["rider_id"]}', style: TextStyle(fontSize: 12, )),),
                  Text('QAR ${list[index]["total_EstimatedFare"]}'?? 'QAR ${(index+1)*6
                }', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),),
                 Text('Cash', style: TextStyle(color: lightGray,fontSize: 13, fontWeight: FontWeight.w500),),
            ...
            Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                   Text(constant_role==0?'Driver Name':'Customer Name', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800),),

                   Text('${(index+1)*3}/${(index+1)*2}/2020,  ${(index+1)*2}:55 ${index.isOdd?'AM':'PM'}', style: TextStyle(fontSize: 14, ),),
                
                ],),
              Column(
                crossAxisAlignment: CrossAxisAlignment.end,
                children: [
                 Text('Trip ID: 049583785', style: TextStyle(fontSize: 14, )),
                  Text('QAR ${(index+1)*6
